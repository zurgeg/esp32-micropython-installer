...
if "--bootfile" in sys.argv:
    ...
    os.system('ampy -p {} put {} main.py'.format(tempoargs[1],tempoargs[2]))
    print('Put the file on memory!')
elif "--copy" in sys.argv:
   tempoargs = sys.argv
   tempoargs.remove('--copy')
   print('Putting {} on ESP32 memory'.format(tempoargs[2]))
   os.system('ampy -p {} put {} {}'.format(tempoargs[1], tempoargs[2],tempoargs[3]))
   print('Put that on memory!')
elif "--get" in sys.argv:
    tempoargs = sys.argv
    tempoargs.remove('--get')
    print('Getting that file')
    out = os.system('ampy -p {} get {}'.format(tempoargs[1],tempoargs[2]))
    print(out)
    ...
    while True:
        ...
        if f.readlines() == prevf:
            os.system('ampy -p {} put {} main.py'.format(tempoargs[2],tempoargs[1]+'/main.py'))
            prevf = f.readlines()
        else:
            #No Changes!
            pass
    ...
    if int(f) == 1:
        ...
        p1 = os.system('esptool.py --port {} erase_flash'.format(sys.argv[1]))
        ...
        p2 = os.system('esptool.py --port {} --baud 460800 write_flash --flash_size=detect -z 0x1000 micropython.bin'.format(sys.argv[1]))
